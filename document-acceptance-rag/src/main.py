"""
FastAPI application with background sheet poller.

Endpoints:
  GET  /health
  POST /process          — process all pending incoming rows
  POST /process?row=N    — process a specific 1-based row number
...
sheets = SheetsClient(
    ...
    credentials_path=cfg["sheets"]["credentials_path"],
)
...
recommender = Recommender(
    ...
    incoming_tab=cfg["sheets"]["incoming_tab"],
...
growth_loop = GrowthLoop(
    ...
    embedding_service=embedding_service,
    novelty_detector=novelty_detector,
    historical_tab=cfg["sheets"]["historical_tab"],
    incoming_tab=cfg["sheets"]["incoming_tab"],
)
...
def _process_pending():
    rows = sheets.get_pending_rows(cfg["sheets"]["incoming_tab"])
    if rows:
...
@app.post("/process")
def process(row: Optional[int] = Query(default=None, description="1-based row number in Incoming_Cases")):
    if row is not None:
        all_rows = sheets.read_incoming(cfg["sheets"]["incoming_tab"])
        # row param is 1-based data row (excluding header)
        idx = row - 1
        if idx < 0 or idx >= len(all_rows):
            return {"error": f"Row {row} out of range (1–{len(all_rows)})"}
        results = [recommender.process_row(all_rows[idx])]
    else:
        pending = sheets.get_pending_rows(cfg["sheets"]["incoming_tab"])
        results = []
        for r in pending:
            try:
...
@app.post("/reindex")
def reindex():
    all_historical = sheets.read_historical(cfg["sheets"]["historical_tab"])
    validated = [r for r in all_historical if r.get("source") == "validated"]
