"""
Seed SQLite DB from data/historical_cases.csv.

Run once before starting the server:
  python scripts/seed_db.py [--csv data/historical_cases.csv]

Also seeds ChromaDB and calibrates the novelty threshold.
"""
...
def coerce_row(row: dict) -> dict:
    """Convert CSV strings to DB-friendly types."""
    bool_fields = {"notarized", "owner_signature", "liens_present"}
    float_fields = {"area_sqm"}
    for field in bool_fields:
        if field in row:
            val = str(row[field]).strip().lower()
            row[field] = 1 if val in ("true", "1", "yes", "نعم") else 0
...
def main():
    ...
    validated = [coerce_row(r) for r in rows if r.get("source") == "validated"]
    print(f"Loaded {len(validated)} validated cases from {args.csv}")

    db = DBClient(cfg["db"]["path"])
    db.seed_historical(validated)
    print(f"Seeded {len(validated)} cases into SQLite → {cfg['db']['path']}")
    ...
    print(f"Indexed into ChromaDB collection: {version}")
